...
def get_dashboard(from_date="", to_date="", user=""):
	...
	for l in layout:
		...
		if l['name'].startswith('lead_status_') and 'status' in l:
			...
			l["data"] = get_lead_status_count(from_date, to_date, user, status_name)
		else:
...
@frappe.whitelist()
@sales_user_only
def get_chart(name, type, from_date="", to_date="", user="", status=None):
	"""
	...
	if name.startswith('lead_status_') and status:
		return get_lead_status_count(from_date, to_date, user, status)
...
def get_feedback_comments(from_date, to_date, user=""):
	...
	prev_feedback = result[0].prev_feedback or 0

	delta_in_percentage = (
		(current_feedback - prev_feedback) / prev_feedback * 100 if prev_feedback else 0
	)
...
def get_leads_by_status(from_date, to_date, user=""):
	"""
...
def get_delayed_leads(from_date, to_date, user=""):
	"""
	...
	prev_delayed = result[0].prev_delayed or 0

	delta_in_percentage = (
		(current_delayed - prev_delayed) / prev_delayed * 100 if prev_delayed else 0
	)
...
def get_total_deals(from_date, to_date, user=""):
	...
	prev_deals = result[0].prev_deals or 0

	delta_in_percentage = (
		(current_deals - prev_deals) / prev_deals * 100 if prev_deals else 0
	)
...
def get_leads_by_status_chart(from_date="", to_date="", user=""):
	"""
